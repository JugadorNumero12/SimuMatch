...
public class Turno {
    ...
    Turno (int estado, Partido p, double aba[]) {
        ...
        if (numeroDeTurno == partido.descanso) {
            comentarista = "Descanso (" + numeroDeTurno + ") ";
        } else {
            comentarista += numeroDeTurno + ": ";
        }
        ...
        switch (estado) {
            case 6:
                ...
                break;
            case 5:
                com(local + " esta dominado el juego, es cuestion de tiempo que marque");
                break;
            case 1:
                com(local + " tiene oportunidad de tomar la iniciativa");
                break;
            case 0:
                com("El partido esta muy igualado");
                break;
            case -1:
                com(visitante + " tiene oportunidad de tomar la iniciativa");
                break;
            case -5:
                com(visitante + " esta dominado el juego, es cuestion de tiempo que marque");
                break;
            case -6:
                com("¡Gol de " + visitante + "!");
                break;
            default:
                ...
                if (estado > 0) {
                    com(local + " lleva la iniciativa");
                } else {
                    com(visitante + " lleva la iniciativa");
                }
        }

        if (partido.animoL > 2 * partido.animoV) {
            com("Parece que " + local + " esta mucho mas animado que " + visitante);
        } else if (partido.animoV > 2 * partido.animoL) {
            com("Parece que " + visitante + " esta mucho mas animado que " + local);
        }

        if (estado > 0) {
            if (partido.tacL == 2) {
                com("Parece que " + local
                    + " no esta aprovechando su ventaja, si yo fuera el entrenador me enfadaria mucho");
            }
            if (partido.tacV == 1) {
                com("Parece que " + visitante
                    + " no esta defendiendo debidamente, es una oportunidad magnifica para marcar");
            }
        } else if (estado < 0) {
            if (partido.tacV == 2) {
                com("Parece que " + visitante
        ...
        if (numeroDeTurno == partido.duracion) {
            ...
            if (partido.goles() == 0) {
                com("EMPATE");
            } else if (partido.goles() > 0) {
                com("Ganador: el equipo local: " + local);
            } else {
                com("Ganador: el equipo visitante: " + visitante);
            }
        }
